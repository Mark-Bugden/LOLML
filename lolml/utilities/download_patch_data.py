...
def main():
    ...
    if not utils.check_config_patch(verbose=False):
        ...
        print(
            f"The config patch ({config_patch}) is out of date! The current LoL patch is {current_patch}\n"
        )
        patch_confirm = input(
            "Would you like to download the config patch or the current patch? Type 'config' to select the config patch, or 'current' to select the current patch: "
        )
    ...
    if check_data_dir_for_patch(patch):
        print(
            "Patch directory already exists! If you want it to be downloaded again, delete the patch directory and restart the script."
        )
        exit()
    else:
